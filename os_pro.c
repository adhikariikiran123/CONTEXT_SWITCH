...
void* threadFunction(void* args) {
	...
	sqlite3 *db;
    if (sqlite3_open("context_switching.db", &db)) {
    fprintf(stderr, "Can't open DB: %s\n", sqlite3_errmsg(db));
    return NULL;
   }

    static const char* filename[4] = {"process1.txt", "process2.txt", "process3.txt", "process4.txt"};
    int size = 4;
    ...
    int pc[] = {0, 0, 0, 0};
    int len[size];
    void* sp[] = {stackpointer(&(stack_p[0])), stackpointer(&(stack_p[1])),
                  stackpointer(&(stack_p[2])), stackpointer(&(stack_p[3]))};
    int t[] = {0, 0, 0, 0};
    int l = 2, u = 7;
    char* state[4] = {"ready", "ready", "ready", "ready"};
    int tot_time = 0;
    ...
    for (int i = 0; i < (tot_time / 2); i++) {
        ...
        if (res[running] == false) {
            if (t[running] < burst_time[running]) {
                ...
                sleep(2);
				insert_process_data(db, process[running], pc[running], state[running], sp[running]);
                ...
                if (file != NULL) {
                    ...
                    while (fgets(line, sizeof line, file) != NULL) {
                        if (count == t[running]) break;
                            ...
                            if (strcmp(string1[0], "add") != 0 || strcmp(string1[0], "sub") != 0 ||
                                strcmp(string1[0], "div") != 0 || strcmp(string1[0], "mult") != 0) {
